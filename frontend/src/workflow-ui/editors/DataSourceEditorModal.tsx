            ...
            <div className="modal modal-editor">
                <div className="modal-header">
                    <div className="modal-title-row"><Database size={16} className="modal-title-icon" /><div>
                        ...
                        <div className="modal-subtitle">Reusable DuckDB catalog reference</div>
                    </div></div>
                ...
                <div className="modal-body">
                    ...
                    <label className="modal-field"><span className="modal-field-label">Type</span><select className="modal-input" value={kind} onChange={e => { setKind(e.target.value as DataSourcePayload['kind']); setConnectionRef(''); }}><option value="duckdb">DuckDB</option><option value="postgres">PostgreSQL</option></select></label>
                    <label className="modal-field"><span className="modal-field-label">Connection</span><select className="modal-input" value={connectionRef} onChange={e => setConnectionRef(e.target.value)}><option value="">Select a compatible connection</option>{compatible.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select></label>
                </div>
                <div className="modal-footer"><button type="button" className="button-secondary" onClick={onCancel}>Cancel</button><button type="button" className="button-primary" disabled={!canSave} onClick={() => onSave(name.trim(), { kind, sqlAlias: sqlAlias.trim(), connectionRef, readOnly: true })}>Save</button></div>
            </div>
        </div>,
        document.body,
    );
